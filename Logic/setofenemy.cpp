...
void SetOfEnemy::Update(float time,sf::RenderWindow &window) // Общий метод для обновления объекта
{
	...
	for (std::vector<Enemys::MovingEnemy>::iterator it = SetOfMovingEnemys.begin(); it != SetOfMovingEnemys.end(); it++) // Пересчитываем координаты врагов и отрисовываем их
	{
...
void SetOfEnemy::UpdateBullets(float time,sf::RenderWindow &window)		    // Обновляем состояние пуль выпущенных врагами
{
